    ...
    <div className="relative">
      <div className="flex border border-nexa-elements-borderColor rounded-md overflow-hidden mr-2 text-sm">
        <Button
          ...
          className="hover:bg-nexa-elements-item-backgroundActive !text-white flex items-center gap-2"
        >
          <img
            className="w-4 h-4"
            height="20"
          ...
          <Dialog className="max-w-[520px] p-6">
            {!isConnected ? (
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-nexa-elements-textSecondary mb-2">Access Token</label>
                  <input
                    type="password"
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 p-3 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg">
                  <div>
                    <h4 className="text-sm font-medium text-nexa-elements-textPrimary">{supabaseConn.user?.email}</h4>
                    <p className="text-xs text-nexa-elements-textSecondary">Role: {supabaseConn.user?.role}</p>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <button
                        ...
                        className="bg-transparent text-left text-sm font-medium text-nexa-elements-textPrimary flex items-center gap-2"
                      >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-2 py-1 rounded-md text-xs bg-[#F0F0F0] dark:bg-[#252525] text-nexa-elements-textSecondary hover:bg-[#E5E5E5] dark:hover:bg-[#333333] flex items-center gap-1"
                    ...
                    {isProjectsExpanded && (
                      <>
                        {!supabaseConn.selectedProjectId && (
                          <div className="mb-2 p-3 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg text-sm text-nexa-elements-textSecondary">
                            ...
                            {supabaseConn.stats.projects.map((project) => (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <div>
                                    <h5 className="text-sm font-medium text-nexa-elements-textPrimary flex items-center gap-1">
                                      <div className="i-ph:database w-3 h-3 text-[#3ECF8E]" />
                                      {project.name}
                                    </h5>
                                    <div className="text-xs text-nexa-elements-textSecondary mt-1">
                                      {project.region}
                                    </div>
                                  </div>
                                  <button
                                    ...
                                    className={classNames(
                                      ...
                                      supabaseConn.selectedProjectId === project.id
                                        ? 'bg-[#3ECF8E] text-white'
                                        : 'bg-[#F0F0F0] dark:bg-[#252525] text-nexa-elements-textSecondary hover:bg-[#3ECF8E] hover:text-white',
                                    )}
                                  >
                                    {supabaseConn.selectedProjectId === project.id ? (
      ...
      className={classNames(
        'flex items-center p-1.5',
        {
          'bg-nexa-elements-item-backgroundDefault hover:bg-nexa-elements-item-backgroundActive text-nexa-elements-textTertiary hover:text-nexa-elements-textPrimary':
            !active,
          'bg-nexa-elements-item-backgroundDefault text-nexa-elements-item-contentAccent': active && !disabled,
